...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label
                          ...
                          WHERE  o.id_shipping = 65 AND track_number IS NOT NULL AND track_number != '' """
    ...
    def success(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  id_shipping in (65) and track_number IS NOT NULL AND track_number != '' and is_settlemented = 0
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_dec(self, item):
        ...
        result = {'track': item['track_number'],
                  ...
                  'loc': ''}
        try:
            url = 'http://42.3.224.83:7028/BGNCXCInterfaceNew/searchScanInfo.do'

            params = {
                'company_id': '98171006',
                'data': '["{0}"]'.format(result['track']).upper()

            }
            
            res = requests.post(url, params=params)
            data = json.loads(res.text)['data']
            status_id = send_date(status, logistics_time, track, ' ', '(65)', '').acquire_date_id()
            if data:
                data_number = len(json.loads(res.text)['data']['{0}'.format(result['track'])])
                i = 0
                for x in range(data_number):
                    status = json.loads(res.text)['data']['{0}'.format(result['track']).upper()][i]['bdBptypecode']
                    logistics_time = json.loads(res.text)['data']['{0}'.format(result['track']).upper()][i]['eventDate']
                    i = i+1
                    tran_status = send_date(status, logistics_time, track, summary_status, '(65)', '').acquire_date()
                    if tran_status:
            ...
            # print json.loads(res.text)['data']['{0}'.format(result['track']).upper()][0]['scanType']
            # print json.loads(res.text)['data']['{0}'.format(result['track']).upper()][0]['eventDate']
            # print json.loads(res.text)['data']['{0}'.format(result['track']).upper()][0]['description']
            # status = json.loads(res.text)['data']['{0}'.format(result['track']).upper()][0]['scanType']
            # logistics_time = json.loads(res.text)['data']['{0}'.format(result['track']).upper()][0]['eventDate']
            # status_second = json.loads(res.text)['data']['{0}'.format(result['track']).upper()][0]['description']
            # print '11111111111111111111111'
            # if status in '完成':
            #     summary_status_label = '已签收'
            #
            # elif status in '出场' or status in '入场':
            #     summary_status_label = '配送中'
            #
            # elif status in '问题件':
            #     summary_status_label = '问题件'
            #
            # elif status in '问题件' and (status_second in '退貨' or status_second in '拒收'):
