...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  o.id_shipping in(196) AND track_number IS NOT NULL AND track_number != '' And is_settlemented = 0 AND o.id_order_status in (8,9,16,23) and o.id_increment != os.track_number"""
    ...
    def success(self):
        ...
        sql = u"""SELECT a.id_order_shipping, a.id_order, a.id_shipping, a.status_label, a.track_number, a.fetch_count ,a.summary_status_label,b.id_increment
                      ...
                      WHERE  a.id_shipping in (196) and a.track_number not like 'SZT%' and a.track_number IS NOT NULL and a.track_number != ''
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_dec(self, item):
        ...
        try:
            ...
            url = 'http://service.nimexpress.com/nss/TrackingPASMIER.htm'
            status_id = send_date(status, logistics_time, track, summary_status, '(196)', '').acquire_date_id()
            parmas = {
                'bill_no': result['track']
            }
            res = requests.get(url, params=parmas)

            print res.text
            i = 0
            for _ in range(len(json.loads(res.text))):
                status = json.loads(res.text)[i]['stepNameEN']
                logistics_time = parse(json.loads(res.text)[i]['dateTime']).strftime('%Y-%m-%d %H:%M:%S')
                tran_status_label = ''
                i = i + 1
                tran_status = send_date(status, logistics_time, track, summary_status, '(196)','').acquire_date()
