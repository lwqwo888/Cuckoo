...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label,o.id_increment
                                                  ...
                                                  WHERE  o.id_shipping in(152,150,108) AND track_number IS NOT NULL AND track_number != '' """
    ...
    def success(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  id_shipping in (152,150,108) and track_number IS NOT NULL AND track_number != '' and is_settlemented = 0
                                  and ( summary_status_label = '已签收')"""
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_dec(self, item):
        ...
        result = {'track': item['track_number'],
                  ...
                  'loc': ''}
        try:
            url = 'http://111.230.142.168:8010/query.aspx'

            params = {
                "__VIEWSTATE": "/wEPDwUKMTg2MTYyMDc5M2RkyxV1MHctQb5TbjgP/inlT008aOE=",
                "__EVENTVALIDATION": "/wEWAwLwruPsDAKvgcuQCQKM54rGBgW1F0FjdWgE6PCmAxYmU5pBx101",
                "__VIEWSTATEGENERATOR": "EDD8C9AE",
                "TrackNum": result['track'],
                "Button1": "追踪"
            }
            res = requests.get(url, params=params)
            result_bad = res.text
            gettime = re.findall(r"<td width=\"130\" height=\"20\">(.*?)</td>", result_bad, re.M | re.S)
            getstat = re.findall(r"<td align=\"left\" colspan=\"8\" height=\"20\">(.*?)</td>", result_bad, re.M | re.S)
            getstat = re.findall(r"<div align=\"left\" style=\"width: 413px\">(.*?)</div>", str(getstat), re.M | re.S)
            i = 0
            k = 0
            j = 1

            status_id = send_date(status, logistics_time, track, 'hello', '(152,150,108)','').acquire_date_id()
            for x in range(len(getstat)):
                gettime_hms    = gettime[j].replace(' ', '').strip()
                gettime_ymd    = gettime[i].replace(' ', '').strip()
                gettime_ymd    = re.findall(r"<divalign=\"left\"style=\"width:96\">(.*?)</div>", str(gettime_ymd),re.M | re.S)
                gettime_hms    = re.findall(r"<divalign=\"left\"style=\"width:80px\">(.*?)</div>", str(gettime_hms),re.M | re.S)
                gettime_ymd    = gettime_ymd[0].replace('\r\n', '')
                gettime_hms    = gettime_hms[0].replace('\r\n', '')
                getstat_result = getstat[k].replace("\\r\\n", "").strip().encode('utf-8').decode('unicode_escape')
                gettime_result = gettime_ymd + ' ' + gettime_hms
                i = i + 2
                j = j + 2
                k = k + 1
                if not getstat_result:
                    getstat_result = "未上线"

                tran_status = send_date(status, logistics_time, track, summary_status, '(152,150,108)', '').acquire_date()
                if tran_status:
                    for tran_status_item in tran_status:
                        if tran_status_item['status_label'].strip() in getstat_result:
                            tran_status_label = tran_status_item['status_label']
                            break
                        else:
                            tran_status_label = getstat_result
                    send_date(getstat_result, gettime_result, track, ' ', str(status_id[0]['id_shipping']),tran_status_label).insert_date()


            
            # 判断状态如果不在数据库的情况
            # if status not in summary_status_key:
            #     send_date(status, logistics_time, track, ' ', '(140,138)').insert_not_exit()
            # else:
            #     summary_status_label = summary_status_dict[status].encode('utf8')
            #
            # if status_id[0]['summary_status_label'] == '拒收':
            #     summary_status_label == "拒收"
            result['status'] = getstat_result
            result['time']   = gettime_result
            #result['summary_status'] = summary_status_label
            print '%s --- %s --- %s --- %s --- %s' % (LOCK_FILE_NAME, result['track'], result['summary_status'], result['status'], result['time'])
            return result
        except Exception, e:
            print e
            return result
