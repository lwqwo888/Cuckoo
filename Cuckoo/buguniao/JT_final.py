#!/usr/bin/env python # -*- coding: utf-8 -*- # version: 0.4 
# 新版ERP 
# 同步物流状态
...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label,o.id_increment
                                                          ...
                                                          WHERE  o.id_shipping in (214,216) AND track_number IS NOT NULL AND track_number != '' """
    ...
    def success(self):
        ...
        sql = u"""SELECT a.id_order_shipping, a.id_order, a.id_shipping, a.status_label, a.track_number, a.fetch_count ,a.summary_status_label,b.id_increment
                              ...
                              WHERE  a.id_shipping in (214,216) and a.is_settlemented=0 and a.track_number != b.id_increment and a.track_number not like 'SZT%' and a.track_number IS NOT NULL and a.track_number != ''
                              and (a.summary_status_label = '已签收')"""
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_taiguo(self, item):
        ...
        status_id = send_date(status, logistics_time, track, 'hello', '(214,216)','').acquire_date_id()
        try:
            url = "http://202.159.30.42:22223/jandt_web/shopee/trackingAction!tracking.action"

            payload = "{\"awb\":\"{0}\"}".format(result['track'])
            headers = {
                'cache-control': "no-cache",
                'postman-token': "c73d9ab2-3980-4cfe-6c0e-1f7868a5ad1d"
            }

            response = requests.request("POST", url, data=payload, headers=headers)

            print json.loads(response.text)["history"]
            from dateutil.parser import parse
            i = 0
            for _ in range(len(json.loads(response.text)["history"])):
                getstat = json.loads(response.text)["history"][i]['status']
                logistics_time = parse(json.loads(response.text)["history"][i]['date_time']).strftime('%Y-%m-%d %H:%M:%S')
                i = i + 1

                tran_status = send_date(getstat, logistics_time, track, summary_status, '(104)', '').acquire_date()
                if tran_status:
            ...
            print '%s --- %s --- %s' % (LOCK_FILE_NAME, getstat, logistics_time)
            return result
        except Exception, e:
            print e
            return result
