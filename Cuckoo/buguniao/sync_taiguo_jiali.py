...
# version: 0.4
# 新版ERP
# 同步物流状态 # 1.新导入的运单号 2.签收 3.退货 4.拒签 5.签收转退货 6.订单状态缺货 7.订单状态取消 # 参数: normal(默认), all(更新所有), success(更新已签收),
# 脚本功能:
...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  o.id_shipping in(14) AND track_number IS NOT NULL AND track_number != '' And is_settlemented = 0 AND o.id_order_status in (8,9,16,23) and o.id_increment != os.track_number"""
    ...
    def success(self):
        ...
        sql = u"""SELECT a.id_order_shipping, a.id_order, a.id_shipping, a.status_label, a.track_number, a.fetch_count ,a.summary_status_label,b.id_increment
                      ...
                      WHERE  a.id_shipping in (14) and a.track_number not like 'SZT%' and a.track_number IS NOT NULL and a.track_number != ''
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_dec(self, item):
        ...
        try:
            ...
            url = 'https://th.kerryexpress.com/th/track/?track=' + result['track']
            ...
            judge_len = len(judge)
            status_id = send_date(status, logistics_time, track, '', '(14)','').acquire_date_id()
            if judge_len:
                ...
                while i <= length:
                    ...
                    time_list1 = []
                    if date_len > 2:
                        date = date_list[2] + '-' + self.month_func(date_list[1].upper()) + '-' + date_list[0]
                    ...
                    for logistics_time, status_str in taiguo_jl_list:
                        ...
                        # print status
                        # tran_status = send_date(status, logistics_time, track, summary_status, '(14)', '').acquire_date()
                        # print tran_status
                        # if tran_status:
