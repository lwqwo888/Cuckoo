...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  o.id_shipping =104 AND track_number IS NOT NULL AND track_number != '' """
    ...
    def success(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  id_shipping in (104) and track_number IS NOT NULL AND track_number != ''
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_dec(self, item):
        ...
        try:
            ...
            url         = 'http://220.132.209.89/API/tm.php?A=tm&C=tm1234&S='+result['track']
            ...
            print (result_data)
            print (result['track'])
            status_id = send_date(status, logistics_time, track, ' ', '(104)').acquire_date_id()
            ...
            if 'information' in result_data:
                ...
                for x in range(data_len):
                    logistics_time = json.loads(result_data)['states_t'][i]
                    ...
                    i = i + 1
                    tran_status = send_date(status, logistics_time, track, summary_status, '(104)', ' ').acquire_date()
                    if tran_status:
                        for tran_status_item in tran_status:
                            if tran_status_item['status_label'] in status:
                ...
                # status = json.loads(result_data)['states_s'][-1]
                # logistics_time = json.loads(result_data)['states_t'][-1]
                # logistics_time = logistics_time[0:4] + '-' + logistics_time[4:6] + '-' + logistics_time[6:8] + '' + \
                #                  logistics_time[8:11] + ':' + logistics_time[11:13] + ':' + logistics_time[13:15]
            ...
            result['summary_status'] = tran_status_label
            print ('%s --- %s --- %s --- %s --- %s' % (
            LOCK_FILE_NAME, result['track'], result['summary_status'], result['status'], result['time']))
            return result
        except Exception as e:
            print (e)
            return result
...
def main():
    ...
    if os.path.exists(lock_file):
        print('%s 模式run_mode 已在运行中...' % lock_file)
        os.remove(lock_file)
        sys.exit(1)
