...
import time
import datetime
from cat_data_base_final import Db_base
...
class send_date(Db_base):
    ...
    def selete_delete_date(self):
        ...
        try:
            ...
            sql += u"INNER JOIN erp_order_shipping as b on a.track_number=b.track_number where b.created_at>= '" + start_time.strftime(
                '%Y-%m-%d 00:00:00') + "'"
            self._cursor.execute(sql)
            self._conn.commit()
            self._conn.close()
    ...
    def insert_date(self):
        ...
        try:
            # cur = self.connection.cursor()
            sql = u"insert into erp_logistics_track(id_shipping,track_number,status_label,date,tran_status_label) VALUES "
    ...
    def insert_date_cat(self):
        ...
        try:
            # cur = self.connection.cursor()
            sql = u"insert into erp_cat_status(track_number,status_label,date_at) VALUES "
    ...
    def delete_cat(self):
        try:
            # cur = self.connection.cursor()
            sql = u"TRUNCATE TABLE erp_cat_status"
            print sql
            self._cursor.execute(sql)
            self._conn.commit()
            self._conn.close()
