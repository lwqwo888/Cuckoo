...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label,o.id_increment
                                                  ...
                                                  WHERE  o.id_shipping in(174,176,270,272,242) AND track_number IS NOT NULL AND track_number != '' """
    ...
    def success(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  id_shipping in(174,176,270,272,242) and track_number IS NOT NULL AND track_number != ''
                                  and (summary_status_label = '拒收')"""
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_taiguo(self, item):
        ...
        try:
            ...
            url = 'http://szerp.stosz.com:8080/Shipping/Param/grap'
            params = {
            ...
            logistics_time = ''
            status_id = send_date(status, logistics_time, track, 'hello', '(174,176,270,272,242)','').acquire_date_id()
            if res.text == -1 or res.text == -5:
                status = '未找到'
                logistics_time = ''
                ...
                i = 0
                for x in range(len(json.loads(res.text))):
                    status            = json.loads(res.text)[i][0].replace('\r\r\n', '')
                    logistics_time    = str(json.loads(res.text)[i][1]).replace('T', ' ')
                    #tran_status_label = json.loads(res.text)[i][2].strip() 
                    i = i + 1
                    tran_status = send_date(status, logistics_time, track, summary_status, '(174,176,270,272,242)', '').acquire_date()
                    if tran_status:
                    ...
                    send_date(status, logistics_time, track, ' ', str(status_id[0]['id_shipping']),tran_status_label).insert_date()
            result['time'] = ''.join(logistics_time).strip()
