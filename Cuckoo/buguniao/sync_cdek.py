#!/usr/bin/env python # -*- coding: utf-8 -*- # version: 0.4 # 新版ERP
# 同步物流状态
...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label
                          ...
                          WHERE  o.id_shipping in(198,200) AND track_number IS NOT NULL AND track_number != '' And is_settlemented = 0 AND o.id_order_status in (8,9,16,23) and o.id_increment != os.track_number"""
    ...
    def success(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
                  ...
                  WHERE id_shipping in(198,200) and track_number not like 'SZT%' and (summary_status_label = '顺利送达' or summary_status_label = '拒收') """
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_datong(self, item):
        ...
        try:
            ...
            url = 'http://cdek-express.cn/ajax.php?JsHttpRequest=' + str_time + '-xml' + '&Action=GetTrackingInfo&invoice=' + result['track']
            ...
            if datetime_list:
               ...
               status_id = send_date(status, logistics_time, track, 'hello', '(198,200)','').acquire_date_id()
