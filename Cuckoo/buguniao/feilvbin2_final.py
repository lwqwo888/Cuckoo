...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label,o.id_increment
                                  ...
                                  WHERE  o.id_shipping in(138,140,110) AND track_number IS NOT NULL AND track_number != '' """
    ...
    def success(self):
        ...
        sql = u"""SELECT a.id_order_shipping, a.id_order, a.id_shipping, a.status_label, a.track_number, a.fetch_count ,a.summary_status_label,b.id_increment
                      ...
                      WHERE  a.id_shipping in (140,138,110) and a.track_number not like 'SZT%' and a.track_number IS NOT NULL and a.track_number != ''
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_dec(self, item):
        ...
        try:
            ...
            # bindIpObj.changeIp("")
            status_id = send_date(status, logistics_time, track, '  ', '(140,138,110)','').acquire_date_id()
                ...
                if data:
                    ...
                    for x in range(len(data)):
                        if data[i]['OperateFlag'] == 'SignConfirm':
                            ...
                            logistics_time = data[i]['OperateTime'].replace('/', '-')[6:10]+'-'+data[i]['OperateTime'].replace('/', '-')[0:5]+data[i]['OperateTime'].replace('/', '-')[10:]
                        i = i + 1
                        tran_status = send_date(status, logistics_time, track, summary_status, '(138,140,110)','').acquire_date()
                        if tran_status:
                            for tran_status_item in tran_status:
                                print tran_status_item['status_label']
            ...
            print '%s --- %s --- %s --- %s' % (
            LOCK_FILE_NAME, result['id_increment'],result['status'], result['time'])
            return result
        except Exception, e:
            print e
            return result
