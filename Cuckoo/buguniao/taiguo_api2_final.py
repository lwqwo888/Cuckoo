...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label,o.id_increment
                                                          ...
                                                          WHERE  o.id_shipping in (144,142,70) AND track_number IS NOT NULL AND track_number != '' """
    ...
    def success(self):
        ...
        sql = u"""SELECT a.id_order_shipping, a.id_order, a.id_shipping, a.status_label, a.track_number, a.fetch_count ,a.summary_status_label,b.id_increment
                              ...
                              WHERE  a.id_shipping in (144,142,70) and a.is_settlemented=0 and a.track_number != b.id_increment and a.track_number not like 'SZT%' and a.track_number IS NOT NULL and a.track_number != ''
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_taiguo(self, item):
        ...
        status_id = send_date(status, logistics_time, track, 'hello', '(144,142,70)','').acquire_date_id()
        try:
            ...
            for taiguo_key in taiguo_keys:
                if not taiguo_values[i]:
                    taiguo_values[i] = ''
                send_date(taiguo_keys[i], taiguo_values[i], track, ' ', str(status_id[0]['id_shipping']),taiguo_keys[i]).insert_date()
                i = i + 1

            print '%s --- %s --- %s' % (LOCK_FILE_NAME, taiguo_keys[0], taiguo_values[0])
            return result
        except Exception, e:
            print e
            return result
