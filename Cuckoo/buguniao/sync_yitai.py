...
class Sync:
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
                  ...
                  WHERE  id_shipping in(156,154) and track_number IS NOT NULL AND track_number != ''
                  and ( summary_status_label != '已签收' and summary_status_label != '拒收' or summary_status_label is NULL ) and is_settlemented = 0"""
    ...
    def all(self):
        ...
        cursor.execute('SET NAMES utf8')
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
              FROM erp_order_shipping WHERE  id_shipping in(156,154) and track_number IS NOT NULL AND track_number != ''"""
        sql += u" AND created_at >= '" + start_time.strftime('%Y-%m-%d 00:00:00') + "'"
        sql += u""" ORDER BY fetch_count ASC, created_at DESC"""
    ...
    def empty(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
              FROM erp_order_shipping
              WHERE id_shipping in(156,154) and track_number IS NOT NULL AND track_number != '' AND (status_label = '' OR status_label IS NULL )
    ...
    def success(self):
        ...
        sql = u"""SELECT id_order_shipping, id_order, id_shipping, status_label, track_number, fetch_count ,summary_status_label
                  FROM erp_order_shipping
                  WHERE id_shipping in(156,154) and summary_status_label = '已签收' and is_settlemented = 0  """
        sql += u" AND created_at >= '" + start_time.strftime('%Y-%m-%d 00:00:00') + "' "
        sql += u""" ORDER BY fetch_count ASC, created_at DESC"""
    ...
    def save_item(self, item):
        try:
            ...
            if data in (4, 5, 7, 18, 22, 24, 25, 26, 27):
                ...
                if item['summary_status'].encode('utf-8') == u'已签收'.encode('utf-8'):
                    ...
                    # 订单从拒收变为签收，状态更改
                    sql_sett_upd = u"""UPDATE erp_order SET refused_to_sign = 0 WHERE id_order='%s' AND `id_order_status` NOT IN (4,5,7,22)""" % (
                    item['id_order'])
                ...
                elif item['summary_status'].encode('utf-8') == u'退貨完成'.encode('utf-8'):
                    sql_order_upd = u"""UPDATE erp_order
                        SET id_order_status=10
                        WHERE id_order='%s' AND `id_order_status` NOT IN (4,5,7,22)""" % item['id_order']
                    cursor_update.execute(sql_order_upd)
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_yitai(self, item):
        ...
        try:
            ...
            res = requests.post("http://tms.etscp.com:8082/selectTrack.htm?documentCode=" + result['track'])
            if json.loads(res.text)[0]['ack'] == 'false':
                gettime = ''
                summary_status = u""
            else:
                getstat = json.loads(res.text)[0]['data'][0]['businessStatus']
                gettime = json.loads(res.text)[0]['data'][0]['trackDate']
                b       = re.compile(r'(.*)\+(.*)')
                gettime = re.search(b,gettime).group(1)
                if getstat in u'转运中':
                    summary_status ='派送中'
                if getstat in u'签收':
                    summary_status ='已签收'
            result['status'] = getstat.strip()
            result['time'] = gettime.strip()
            result['summary_status'] = summary_status.strip()
...
def main():
    ...
    lock_file = '.fetch_yitai_' + run_mode
    if os.path.exists(lock_file):
        print ('%s 模式已在运行中...' % lock_file)
        os.remove(lock_file)
        sys.exit(1)
    file_hide = open(lock_file, 'w')
