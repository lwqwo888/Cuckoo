...
def mian():
    ...
    try:
        ...
        for line in lines:
            ...
            logistics_time = parse(logistics_time).strftime('%Y-%m-%d %H:%M:%S')
            ...
            if len(datas_line) == 500:
                send_date().insert_date_cat(datas_line)
                page_line += 1
                datas_line = []

            elif len(datas_line) < 500:
    ...
    for select__final_data in select__final_datas:
        datas.append((str(select__final_data['id_shipping']), str(select__final_data['track_number']),
                      select__final_data['status_label'], str(select__final_data['date_at']), select__final_data['status_label']))
        if len(datas) == 1000:
