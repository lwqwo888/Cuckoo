...
class Sync:
    def __init__(self, host, user, passwd, db_name, mode='normal', thread_count=5):
        self.user = user
        self.passwd = passwd
    ...
    def normal(self):
        ...
        sql = u"""SELECT id_order_shipping, o.id_order, o.id_shipping, status_label, track_number, fetch_count ,summary_status_label
                                  ...
                                  WHERE  o.id_shipping in(186,188,210,212,82,268) AND track_number IS NOT NULL AND track_number != '' And is_settlemented = 0 AND o.id_order_status in (8,9,16,23) and o.id_increment != os.track_number"""
    ...
    def success(self):
        ...
        sql = u"""SELECT a.id_order_shipping, a.id_order, a.id_shipping, a.status_label, a.track_number, a.fetch_count ,a.summary_status_label,b.id_increment
                      ...
                      WHERE  a.id_shipping in (186,188,210,212,82,268) and a.track_number not like 'SZT%' and a.track_number IS NOT NULL and a.track_number != ''
...
class SyncStatusThread(threading.Thread):
    ...
    def fetch_dec(self, item):
        ...
        try:
            ...
            url = 'http://edi.gdexapi.com/api/key/PreAlert/DeliveryStatus/' + result['track']
            params = {
            ...
            status_id = send_date(status, logistics_time, track, '', '(186,188,210,212,82,268)','').acquire_date_id()
            if time_list:
                ...
                for status_code, df_code, logistics_time in gdex_list:
                    if df_code != "null":
                        ...
                        status = self.control_df(df_code)
                        tran_status_label = df_code
                    else:
                        status = self.control(status_code)
                        tran_status_label = status_code
                    # print logistics_time + '\n' + status + '\n' + tran_status_label + '\n'
                    tran_status = send_date(status, logistics_time, track, summary_status, '(186,188,210,212,82,268)', '').acquire_date()
                    # print tran_status
                    if tran_status:
