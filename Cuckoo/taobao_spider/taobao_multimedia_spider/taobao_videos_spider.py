...
class Taobao_Videos(object):
    def __init__(self):
        ...
        self.__file_log = open('%s/videos_log%s.txt' % (current_path, time.strftime('%Y_%m_%d')), 'a+')
    ...
    def download_video(self, sid, vid1, vid2):
        path = "taobao_multimedia_data/%s/videos/" % (sid)
        ...
        try:
            ...
            with open("taobao_multimedia_data/" + sid + "/videos/" + sid + "video.mp4", 'wb') as f:
                f.write(res)
            print '[INFO]: %s视频保存成功!!\n' % sid
        except Exception as e:
            print "视频保存失败!"
